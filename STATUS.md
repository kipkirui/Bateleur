...
**Send.** Confirm-gated SMTP via `lettre` (STARTTLS on 587, implicit TLS on 465). Compose / Reply / mailto: pick a From mailbox, then **Send** then **Confirm send**. The same OS-keychain password is used. After SMTP succeeds, IMAP accounts APPEND the RFC822 to Sent (`\Seen`). POP accounts keep a local Sent copy. Gmail/Outlook still need an app password. Empty body or a missing To address is refused.
