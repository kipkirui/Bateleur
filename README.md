...
<p align="center">
  ...
  ·
  <a href="./prebuilt/windows/Bateleur_0.27.5_x64_en-US.msi">Try Windows</a>
  ·
...
You do not need Node, Rust, or a compile. Unsigned 0.27.5 builds are in [`prebuilt/windows`](./prebuilt/windows).

1. Download **[Bateleur_0.27.5_x64_en-US.msi](https://github.com/kipkirui/Bateleur/raw/main/prebuilt/windows/Bateleur_0.27.5_x64_en-US.msi)** (preferred) and open it.
2. Windows SmartScreen / Defender may warn. These files are not Authenticode-signed; that is expected. Prefer the `.msi`. The NSIS [setup `.exe`](https://github.com/kipkirui/Bateleur/raw/main/prebuilt/windows/Bateleur_0.27.5_x64-setup.exe) is the one Defender often quarantines.
...
Installers (Windows NSIS/MSI, macOS `.dmg`, Linux AppImage/`.deb`) come from tagging `vX.Y.Z` — see [`CONTRIBUTING.md`](./CONTRIBUTING.md). Local: `npm run bundle` with `TAURI_SIGNING_PRIVATE_KEY_PATH` pointing at the updater private key. Settings can check GitHub for updates; Install is explicit.
