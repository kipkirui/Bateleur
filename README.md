...
Keyboard: `j` / `k` move, `Enter` open, `r` reply, `a` reply all, `f` forward, `c` or `n` compose, `Ctrl+K` search, `Esc` close. In Compose, **Focus** fills the window; type `::thanks` or `/followup` for a snippet. **Cc** and **Bcc** sit behind those labels until you need them; Reply all puts extra people on Cc.
